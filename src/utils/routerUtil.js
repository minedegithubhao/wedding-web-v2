...
import { UserLayout, BasicLayout, RouteView, BlankLayout, PageView, PortalLayout, PTSLayout } from '@/layouts'
...
const constantRouterComponents = {
  ...
  PTSLayout: PTSLayout, // PTS专属基础页面布局，包含了头部，底部导航
  ...
  PortalLayout: PortalLayout, // 门户页面的通用布局
  ...
  portalIndex: () => import('@/views/account/settings/PortalIndex'),
 ...
 * @returns {Promise<any>}
 */
export const generatorDynamicRouter = parameter => {
  return new Promise((resolve, reject) => {
    // ajax
    getRouterByUser(parameter)
      .then(res => {
        const routersByTopMenu = {}
        for (const key in res) {
          const rootMenu = getRootMenu(res[key])
          const routers = generator(rootMenu)
          // routers.push(notFoundRouter)
          routersByTopMenu[key] = routers
        }
        resolve({ routersByTopMenu, allTopMenus: res })
      })
      .catch(err => {
        reject(err)
      })
 ...
 * 获取主菜单信息
 */
export const generatorGetTopMenu = () => {
  return new Promise((resolve, reject) => {
    // ajax
    getTopMenu()
      .then(res => {
...
export function getRootMenu(rows) {
  ...
  const rootMenu = []
  const arr = []
  const menus = defaultRouterMap.concat()
  buildtree(rows, arr, 0)
  if (arr[0] && arr[0].key === 'portalClient') {
    rootRouter.component = 'BlankLayout'
    rootRouter.children = arr
