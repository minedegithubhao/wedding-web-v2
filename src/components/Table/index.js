...
export default {
  ...
  methods: {
          ...
          if (column.dataIndex !== 'action') {
            column.customCell = record => {
              return {
                on: {
                  // 事件
                  click: event => this.clickColumns(record, event) // 点击行
                }
              }
            }
          }
    ...
    clickColumns(selectedRow, event) {
      if (this.rowSelection && this.rowSelection.type !== 'radio') {
        if (this.selectedRowKeys.indexOf(selectedRow[this.rowKey]) > -1) {
          this.selectedRows.splice(
            this.selectedRows.findIndex(item => item[this.rowKey] === selectedRow[this.rowKey]),
            1
          )
          this.selectedRowKeys.splice(
            this.selectedRowKeys.findIndex(item => item === selectedRow[this.rowKey]),
            1
          )
        } else {
          this.selectedRows.push(selectedRow)
          this.selectedRowKeys.push(selectedRow[this.rowKey])
        }
      } else {
        this.selectedRowKeys = [selectedRow[this.rowKey]]
  ...
  render() {
    ...
    if (props.rowSelection.selectedRowKeys.length !== this.selectedRowKeys.length) {
      this.selectedRowKeys = props.rowSelection.selectedRowKeys
      this.selectedRows = []
      props.dataSource.forEach(row => {
        if (props.rowSelection.selectedRowKeys.includes(row[this.rowKey])) {
