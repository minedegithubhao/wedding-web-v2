...
// topMenu
export function getSysTopMenuList(parameter) {
  return axios({
    url: api.sysTopMenu + '/list',
    method: 'get',
    params: parameter
  })
}

export function saveSysTopMenu(parameter) {
  return axios({
    url: api.sysTopMenu + (parameter.id > 0 ? '/update' : '/save'),
    method: 'post',
    data: parameter,
    headers: {
      'Content-Type': 'application/json;charset=UTF-8'
...
export function delSysTopMenu(parameter) {
  return axios({
    url: api.sysTopMenu + '/remove',
    method: 'post',
    params: parameter
  })
}

export function getMenuPermIds(topMenuId) {
  return axios({
    url: api.permission + '/topMenu/' + `${topMenuId}`,
    method: 'get'
  })
}
