  ...
  <a-modal title="操作"
    ...
    <a-card :bordered="false">
      ...
      <detail-list title=""
                   size="small"
                   :col="1">
        <detail-list-item term="登录信息">{{ mdl.operName }}/{{ mdl.operIp }}/{{ mdl.operLocation }}</detail-list-item>
      </detail-list>
      <detail-list title=""
                   size="small"
                   :col="1">
      ...
      <detail-list title="参数"
                   size="small">
        <a-textarea :rows="15"
                    v-model="mdl.operParam"
                    disabled
                    style="color:#000" />
      </detail-list>
      <a-divider style="margin-bottom: 32px" />
...
export default {
  ...
  methods: {
    detail (record) {
      ...
      this.mdl.operType = this.operTypeMap[this.mdl.businessType] && this.operTypeMap[this.mdl.businessType].text || this.mdl.businessType
      this.visible = true
    }
  }
}
